...
void check_function(struct node *function) {
    ...
    // The parameter list is the first child of a function
    check_parameters(getchild(function, 1), scope); // Add's parameters to the local scope
    check_expression(getchild(function, 2), scope); 

    // Add the scope to the queue of scopes
